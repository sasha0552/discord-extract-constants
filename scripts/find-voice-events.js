...
function main() {
    ...
    for (const fileName of fs.readdirSync(constants.CONSTS_RESOLVER)) {
        ...
        awalk.full($, (node) => {
            let isVoiceEventsNode = false;

            /////

            if (constants.COMPARER_RESOLVER(node)) {
                for (const stmt of node.argument.callee.body.body) {
                    if (constants.COMPARER_SPEAKING_IN_VOICE_EVENTS(stmt)) {
                        isVoiceEventsNode = true;
                    }
                }
            }
    ...
    for (let i = 0; i < results.length; i++) {
        fs.writeFileSync(path.join(constants.CONSTS_RESULTS, `voice-events-${i}.json`), JSON.stringify(results[i]));
    }    
}
